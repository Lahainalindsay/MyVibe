...
function parseArgs(argv) {
  const args = { file: undefined, image: undefined };
...
function buildDefaultSVGDataUri() {
...
async function main() {
  ...
  if (!image) image = buildDefaultSVGDataUri();

  const t = await wyv.setPreRevealImage(image);
  console.log("setPreRevealImage tx:", t.hash);
  await t.wait();
  const stored = await wyv.preRevealImage();
  console.log("preRevealImage length:", stored.length);
}
