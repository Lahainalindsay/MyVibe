...
contract WhatsYourVibeNFT is ERC721Enumerable, Ownable {
    ...
    string public preRevealImage; // data:image/svg+xml;base64,... or external URL
        ...
        Ownable(_owner)
    {
        ...
        // Minimal default placeholder image (small SVG) to keep bytecode size low.
            ...
            abi.encodePacked(
                ...
                Base64.encode(
                    bytes(
                        ...
                        '<defs><radialGradient id="bg" cx="50%" cy="35%" r="70%">'
                        '<stop offset="0%" stop-color="#1a1a23"/><stop offset="100%" stop-color="#0a0a0f"/>'
                        '</radialGradient></defs>'
                        ...
                        '<text x="50%" y="255" text-anchor="middle" font-size="20" fill="#ffffff" opacity="0.9" font-family="sans-serif">what\'s your vibe?</text>'
                        '</svg>'
                    )
                )
            )
    ...
    function setPreRevealImage(string calldata dataUri) external onlyOwner {
        // Expect a data URI (e.g., data:image/svg+xml;base64,...) or https URL
        preRevealImage = dataUri;
    }
    ...
    function _preRevealTokenURI(uint256 tokenId) internal view returns (string memory) {
        ...
        string memory image = preRevealImage;
